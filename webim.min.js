...
{_init:function(){var a=this.data,b=this.options.key,c=o.localStorage;if(c)try{c.setItem("__store_webim__","__store_webim__");if(c.getItem("__store_webim__")=="__store_webim__")this.store=c;c.removeItem("__store_webim__")}catch(d){this.store=M}if(a)this._save(a);else this.data=(a=this.store?this.store.getItem(b):da(b))?V.parse(a):{}},set:function(a,b){var c=a;if(typeof a=="string"){c={};c[a]=b}var d=this.data;Y(d,c)&&this._save(u({},d,c))},get:function(a){return this.data[a]},clear:function(){this._save({})},
...
a=R(a)?a:[a];for(var c in a){var d=a[c];d.presence=d.presence=="offline"?"offline":"online";d.incomplete=!b[d.id];if(!d.group&&d.id)d.group=d.id.indexOf("vid:")==0?"visitor":d.group}this.set(a)},load:function(a){a=ea(a);a.length&&s({type:"get",url:r("buddies"),cache:false,data:{ids:a.join(","),csrf_token:m.csrf_token},context:this,success:this.set})},set:function(a){var b=this.data,c=this.dataHash,d={};a=a||[];for(var f=a.length,g,i,l,k=0;k<f;k++){g=a[k];if(l=g.id){if(!c[l]){g.presence=g.presence||
"online";g.show=g.show?g.show:g.presence=="offline"?"unavailable":"available";c[l]={};b.push(c[l])}g.incomplete=!!g.incomplete;if(i=Y(c[l],g)){g=i.presence||"update";d[g]=d[g]||[];u(c[l],i);d[g].push(c[l])}}}for(var v in d)this.trigger(v,[d[v]]);this.options.active&&this.complete()}});K("presence",{},{_init:function(){this.data=this.data||{};this.set(this.data)},get:function(a){return this.data[a]},set:function(a){var b={};for(var c in a){var d=a[c],f="online";if(d=="unavailable"||d=="invisible")f=
"offline";b[f]=b[f]||[];b[f].push({id:c,show:d})}for(var g in b)this.trigger(g,[b[g]])},update:function(a){for(var b={},c=0;c<a.length;c++){var d=a[c];b[d.from]=d.show}this.set(b)},clear:function(){this.data=[]}});(function(){K("room",{},{_init:function(){this.data=this.data||[];this.dataHash={}},get:function(a){return this.dataHash[a]},all:function(a){return a?O(this.data,function(b){return b.temporary}):this.data},invite:function(a,b,c,d){var f=this,g=f.options;s({type:"post",cache:false,url:r("invite"),
data:{ticket:g.ticket,id:a,nick:b||"",members:c.join(","),csrf_token:m.csrf_token},success:function(i){f.set([i]);f.loadMember(a);d&&d(i)}})},join:function(a,b,c){var d=this,f=d.options;s({type:"post",cache:false,url:r("join"),data:{ticket:f.ticket,id:a,nick:b||"",csrf_token:m.csrf_token},success:function(g){d.set([g]);d.loadMember(a);c&&c(g)}})},leave:function(a){var b=this,c=b.options,d=b.dataHash[a],f=c.user;d&&s({type:"post",cache:false,url:r("leave"),data:{ticket:c.ticket,id:a,nick:f.nick,temporary:d.temporary,
...
[];F(b.dataHash,function(g,i){!i.temporary&&i.blocked&&f.push(i.id)});s({type:"post",cache:false,url:r("unblock"),data:{ticket:c.ticket,id:a,csrf_token:m.csrf_token},success:function(){b.trigger("unblocked",[a,f])}})}},set:function(a){var b=this,c=b.data,d=b.dataHash;F(a,function(f,g){var i=g.id;if(i){g.members=g.members||[];g.all_count=g.members.length;g.count=0;F(g.members,function(l,k){if(k.presence=="online")g.count+=1});if(d[i])u(d[i],g);else{d[i]=g;c.push(g)}b.trigger("updated",d[i])}})},loadMember:function(a){var b=
this,c=b.options;s({type:"get",cache:false,url:r("members"),data:{ticket:c.ticket,id:a,csrf_token:m.csrf_token},success:function(d){b.updateMember(a,d)}})},updateMember:function(a,b){var c=this.dataHash[a];if(c){c.memberLoaded=true;c.members=b;this.set([c])}},onPresence:function(a){var b=a.type;if(a.to&&this.dataHash[a.to]){var c=a.to,d=this.dataHash[c];d&&d.memberLoaded&&this.loadMember(c);if(b=="join")this.trigger("memberJoined",[c,a]);else if(b=="leave")this.trigger("memberLeaved",[c,a]);else if(b==
"grponline")this.trigger("memberOnline",[c,a]);else b=="grpoffline"&&this.trigger("memberOffline",[c,a])}},clear:function(){this.data=[];this.dataHash={}}})})();K("history",{},{_init:function(){this.data=this.data||{};this.data.chat=this.data.chat||{};this.data.grpchat=this.data.grpchat||{}},clean:function(){this.data.chat={};this.data.grpchat={}},get:function(a,b){return this.data[a][b]},set:function(a){var b=this.data,c={chat:{},grpchat:{}};a=ca(a);var d=a.length,f,g,i=this.options.userInfo.id;
if(d){for(var l=0;l<d;l++){f=a[l];k=f.type;if((g=k=="chat"?f.to==i?f.from:f.to:f.to)&&k){c[k][g]=c[k][g]||[];c[k][g].push(f)}}for(var k in c)for(g in c[k]){f=c[k][g];if(b[k][g]){b[k][g]=[].concat(b[k][g]).concat(f);this._triggerMsg(k,g,f)}else this.load(k,g)}}},_triggerMsg:function(a,b,c){this.trigger(a,[b,c])},clear:function(a,b){this.data[a][b]=[];this.trigger("clear",[a,b]);s({url:r("clear"),type:"post",cache:false,data:{type:a,id:b,csrf_token:m.csrf_token}})},download:function(a,b){var c=r("download"),
d=t.createElement("iframe"),f=new Date,g=[];f={id:b,type:a,time:(new Date).getTime(),date:f.getFullYear()+"-"+(f.getMonth()+1)+"-"+f.getDate()};for(var i in f)g[g.length]=encodeURIComponent(i)+"="+encodeURIComponent(f[i]);c+=(/\?/.test(c)?"&":"?")+g.join("&");d.setAttribute("src",c);d.style.display="none";t.body.appendChild(d)},init:function(a,b,c){if(R(c)){this.data[a][b]=c;this._triggerMsg(a,b,c)}},load:function(a,b){var c=this;c.data[a][b]=[];s({url:r("history"),cache:false,type:"get",data:{type:a,
id:b,csrf_token:m.csrf_token},success:function(d){c.init(a,b,d)}})}})})(window,document);
