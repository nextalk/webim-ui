...
{id:j.from,presence:j.type};if(j.show)o.show=j.show;if(j.nick)o.nick=j.nick;if(j.status)o.status=j.status;return o}function b(j){return j.type=="online"||j.type=="offline"||j.type=="show"}function c(j){return j.type=="invite"||j.type=="join"||j.type=="leave"}var d=this,f=d.history,g=d.buddy,i=d.room;d.bind("message",function(j,o){for(var A=[],aa=o.length,Q=d.data.user.id,y,E,U,p=0;p<aa;p++){y=o[p];U=y.type;E=U=="chat"?y.to==Q?y.from:y.to:y.to;y.id=E;if(U=="chat"&&!y["new"]){E={id:E,presence:"online"};
if(y.nick&&y.to==Q)E.nick=y.nick;A.push(E)}}if(A.length){g.presence(A);g.complete()}f.set(o)});d.bind("presence",function(j,o){g.presence(Z(N(o,b),a));o=N(o,c);for(var A=o.length-1;A>=0;A--)i.onPresence(o[A])})},handle:function(a){if(a.messages&&a.messages.length){for(var b=a.messages,c=[],d=[],f=0;f<b.length;f++){var g=b[f];if(g.body&&g.body.indexOf("webim-event:")==0){g.event=g.body.replace("webim-event:","").split("|,|");d.push(g)}else c.push(g)}c.length&&this.trigger("message",[c]);d.length&&
...
success:this.set})},set:function(a){var b=this.data,c=this.dataHash,d={};a=a||[];for(var f=a.length,g,i,j,o=0;o<f;o++){g=a[o];if(j=g.id){if(!c[j]){g.presence=g.presence||"online";g.show=g.show?g.show:g.presence=="offline"?"unavailable":"available";c[j]={};b.push(c[j])}g.incomplete=!!g.incomplete;if(i=Y(c[j],g)){g=i.presence||"update";d[g]=d[g]||[];u(c[j],i);d[g].push(c[j])}}}for(var A in d)this.trigger(A,[d[A]]);this.options.active&&this.complete()}});K("presence",{},{_init:function(){this.data=this.data||
...
cache:false,url:r("leave"),data:{ticket:c.ticket,id:a,nick:f.nick,temporary:d.temporary,csrf_token:l.csrf_token},success:function(){delete b.dataHash[a];b.trigger("leaved",[a])}})},block:function(a){var b=this,c=b.options,d=b.dataHash[a];if(d&&!d.blocked){d.blocked=true;var f=[];F(b.dataHash,function(g,i){!i.temporary&&i.blocked&&f.push(i.id)});s({type:"post",cache:false,url:r("block"),data:{ticket:c.ticket,id:a,csrf_token:l.csrf_token},success:function(){b.trigger("blocked",[a,f])}})}},unblock:function(a){var b=
this,c=b.options,d=b.dataHash[a];if(d&&d.blocked){d.blocked=false;var f=[];F(b.dataHash,function(g,i){!i.temporary&&i.blocked&&f.push(i.id)});s({type:"post",cache:false,url:r("unblock"),data:{ticket:c.ticket,id:a,csrf_token:l.csrf_token},success:function(){b.trigger("unblocked",[a,f])}})}},set:function(a){var b=this,c=b.data,d=b.dataHash;F(a,function(f,g){var i=g.id;if(i){g.members=g.members||[];g.all_count=g.members.length;g.count=0;F(g.members,function(j,o){if(o.presence=="online")g.count+=1});
